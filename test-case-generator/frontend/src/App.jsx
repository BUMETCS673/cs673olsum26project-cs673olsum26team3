...
export default function App() {
  ...
  const fetchProjects = async () => {
    try {
      const res = await authorizedRequest(`/api/projects?userId=${user.id}`);
  ...
  const handleCreateProject = async (projectData) => {
    try {
      const res = await authorizedRequest('/api/projects', {
        method: 'POST',
  ...
  const handleDeleteProject = async (id) => {
    try {
      const res = await authorizedRequest(`/api/projects/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
